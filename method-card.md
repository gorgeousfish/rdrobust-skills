...
This is a local Wald (IV) estimator; identifies LATE for compliers at the cutoff.
...
where \(\hat{B}\) is the estimated leading bias term.

### Robust Variance Estimation

The robust bias-corrected (RBC) confidence interval accounts for the additional variability introduced by bias estimation:

\[CI_{\text{RBC}} = \left[\hat{\tau}_{\text{bc}} \pm z_{\alpha/2} \cdot \hat{\sigma}_{\text{rb}}\right]\]

where \(\hat{\sigma}_{\text{rb}}^2 = \text{Var}(\hat{\tau}_{\text{bc}})\) incorporates the uncertainty in both the treatment effect estimator and the bias estimator.

Key result from CCT (2014): conventional CIs using \(\hat{\tau}\) with standard SE have *incorrect* coverage. The RBC CI achieves correct coverage uniformly.
...
The rate is \(h_{\text{MSE}} \sim n^{-1/(2p+3)}\), so for local linear (p=1): \(h \sim n^{-1/5}\).
...
CER bandwidth is typically smaller than MSE bandwidth (more conservative).
...
| `msecomb1` | min(mserd, msesum) | Conservative MSE |
| `msecomb2` | median(msetwo, mserd, msesum) | Robust MSE |
| `cerrd` | Common CER-optimal | Coverage-focused |
| `certwo` | Two CER-optimal (left/right) | Asymmetric + coverage |
| `cersum` | CER-optimal for sum | Sum + coverage |
| `cercomb1` | min(cerrd, cersum) | Conservative CER |
| `cercomb2` | median(certwo, cerrd, cersum) | Robust CER |
...
| Component | Rate | Notes |
|-----------|------|-------|
| Point estimator | \(n^{-(p+1)/(2p+3)}\) bias; \(n^{-1/2}h^{-1/2}\) std dev | For local linear (p=1): \(n^{-2/5}\) bias, \(n^{-2/5}\) RMSE |
| MSE-optimal bandwidth | \(h \sim n^{-1/(2p+3)}\) | For p=1: \(h \sim n^{-1/5}\) |
| CER-optimal bandwidth | \(h \sim n^{-1/(2q+3)}\) | Typically smaller than MSE |
| Bias-corrected estimator | Additional \(b^{-(q+1)}\) term | Accounts for bias estimation error |
